...
try:
    from stock_scanner_v5_upgrade import V5Integration
    V5_AVAILABLE = True
except ImportError:
    V5_AVAILABLE = False
...
class StockScanner:
        ...
        if self.v5:
            ...
            try:
                ...
                if emotion_v2:
                    ...
                    emotion["stage"] = emotion_v2.get("stage", emotion["stage"])
                    emotion["aggression"] = aggression
                    print(f"  📊 V5情绪增强: {emotion_v2.get('stage','')} "
                          f"炸板率={emotion_v2.get('broken_rate',0):.1%} "
                          f"连板高度={emotion_v2.get('consecutive_board_height',0)}")

                # 板块轮动信息
                rotation = v5_results.get("sector_rotation", {})
                if rotation.get("today_hot_sectors"):
                    print(colored(f"  🔄 板块轮动: {rotation.get('rotation_phase','')} — "
                                  f"{rotation.get('rotation_direction','')}", C.CYAN))
                    for name, info in rotation["today_hot_sectors"][:3]:
                        print(f"    {name}: 热度{info['heat_score']:.0f} "
                              f"涨停{info['limit_ups']} 均涨{info['avg_pct']:+.2f}%")
                ...
                for s, r1, r2 in results:
                    try:
                        ...
                        if v5_score_result.get("veto"):
                            ...
                            old_score = r1.final_score
                            r1.final_score = v5_score_result["final_score"]
                            if v5_score_result["multiplier"] > 1.1:
                                boosted_count += 1
                            if v5_score_result["multiplier"] != 1.0:
                                r1.adjust_reason += f" [V5×{v5_score_result['multiplier']:.2f}]"

                        s.enhanced["v5"] = v5_score_result.get("v5_detail", {})
                        s.enhanced["v5_score"] = v5_score_result
                    except Exception as e:
                        print(f"  ⚠ V5评分异常 {s.code}: {e}")

                print(f"  ✅ V5评分完成: 否决{veto_count}只, 大幅加分{boosted_count}只")
    ...
    def _print_results(self, results: list, market: dict, emotion: dict, effective_min: float,
        ...
        # v5.0: 输出V5增强信息
        if any(s.enhanced.get("v5_score") for s, _, _ in qualified):
            print(colored("\n" + "-" * 70, C.DIM))
            print(colored("  🚀 V5 非线性评分详情", C.BOLD))
            print(colored("-" * 70, C.DIM))
            for s, r1, r2 in qualified[:5]:
                v5 = s.enhanced.get("v5_score", {})
                if v5:
                    veto_str = " ❌已否决" if v5.get("veto") else ""
                    mult = v5.get("multiplier", 1.0)
                    mult_str = f" ×{mult:.2f}" if mult != 1.0 else ""
                    adj = ", ".join(v5.get("adjustments", []))
                    print(f"  {s.code} {s.name[:6]}: "
                          f"乘数{mult_str}{veto_str} "
                          f"[{adj}]" if adj else "")
    ...
    def _export_csv(self, results: list):
        ...
        for s, r1, r2 in results:
            ...
            rows.append({
                ...
                # v5.0 字段
                "V5乘数": eh.get("v5_score", {}).get("multiplier", 1.0),
                "V5否决": "是" if eh.get("v5_score", {}).get("veto") else "",
                "V5否决原因": eh.get("v5_score", {}).get("veto_reason", ""),
                "V5调整": "|".join(eh.get("v5_score", {}).get("adjustments", [])),
            })
    ...
    def _export_html(self, results: list, market: dict, emotion: dict):
        ...
        for i, (s, r1, r2) in enumerate(qualified, 1):
            ...
            # v5.0 badges
            v5_score = eh.get("v5_score", {})
            if v5_score.get("veto"):
                enhanced_badges += f'<span class="badge warn">V5否决</span>'
            elif v5_score.get("multiplier", 1.0) > 1.2:
                enhanced_badges += f'<span class="badge hot">V5×{v5_score["multiplier"]:.1f}</span>'
...
<body>
  ...
  <div class="summary">
    ...
    <div class="stat"><div class="num">{sum(1 for s,_,_ in qualified if s.enhanced.get("v5_score",{}).get("veto"))}</div><div class="label">V5否决</div></div>
    <div class="stat"><div class="num">{sum(1 for s,_,_ in qualified if s.enhanced.get("v5_score",{}).get("multiplier",1.0)>1.1)}</div><div class="label">V5大幅加分</div></div>
  </div>
