...
import sys
import io
# ── Windows GBK 编码修复：让 emoji 和中文能正常打印 ──
if sys.platform == 'win32' and sys.stdout.encoding and sys.stdout.encoding.lower() != 'utf-8':
    sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace')
if sys.platform == 'win32' and sys.stderr.encoding and sys.stderr.encoding.lower() != 'utf-8':
...
# mootdx专用线程池（限制并发数，避免TCP连接爆炸阻塞事件循环）
_mootdx_executor = None

# mootdx TCP 探测 + 三级 fallback（兼容 0.10/0.11，规避 BESTIP.HQ 空串 bug）
_TDX_SERVERS = [
...
def _tdx_client(market='std'):
    ...
    顺序兜底：TCP探测→bestip测速→裸factory→明确报错
    """
    from mootdx.quotes import Quotes as _Q
    for ip, port in _TDX_SERVERS:
        if _tdx_probe(ip, port):
            return _Q.factory(market=market, server=(ip, port))
    try:
        return _Q.factory(market=market, bestip=True)
    except Exception:
        pass
    try:
        return _Q.factory(market=market)
    except Exception as e:
        raise RuntimeError(f"所有 mootdx 服务器均不可达: {e}")

try:
    _MOOTDX_CLIENT = _tdx_client(market='std')
    _HAS_MOOTDX = True
    # mootdx专用线程池：最多4个并发TCP连接（避免连接数爆炸）
    from concurrent.futures import ThreadPoolExecutor as _TPE
    _mootdx_executor = _TPE(max_workers=4, thread_name_prefix="mootdx")
except Exception:
    _HAS_MOOTDX = False
    _MOOTDX_CLIENT = None
...
class ProxyPool:
    """
    轻量级代理池：自动轮换 + 失败剔除 + 降级直连
    
    用法:
      pool = ProxyPool()                           # 空池（直连模式）
      pool = ProxyPool(["http://1.2.3.4:8080"])    # 手动代理
      pool = ProxyPool.from_file("proxies.txt")    # 从文件加载
      pool = ProxyPool.from_free_sources()         # 自动抓取免费代理
    
      proxy = pool.get()   # 获取下一个可用代理（None=直连）
      pool.mark_bad(proxy) # 标记代理不可用
    """
    
    def __init__(self, proxies: list[str] = None, rate_limit: float = 0):
        """
        初始化代理池
        
        Args:
            proxies: 代理URL列表
            rate_limit: 直连模式下每次请求间隔（秒），0=不限速
                        当无代理可用时，通过限速避免被东财封IP
                        推荐: 1.0~2.0秒（实测0.5秒以下容易触发封禁）
        """
        self._proxies = list(proxies) if proxies else []
        self._index = 0
        self._bad = {}           # {proxy: fail_count} 失败计数
        self._bad_threshold = 3     # 连续失败N次后剔除
        self._lock = threading.Lock()
        self._direct_fails = 0      # 直连失败次数
        self._direct_fails_threshold = 5  # 直连连续失败后自动尝试切换代理
        self._rate_limit = rate_limit  # 限速间隔（秒）
        self._last_request_time = 0   # 上次请求时间戳
        # ── 运行统计 ──
        self._stats = {
            "total_requests": 0,    # 总请求数
            "proxy_requests": 0,    # 代理请求数
            "direct_requests": 0,   # 直连请求数
            "proxy_success": 0,     # 代理成功数
            "proxy_fail": 0,        # 代理失败数
            "direct_success": 0,    # 直连成功数
            "direct_fail": 0,       # 直连失败数
            "proxies_removed": 0,   # 被剔除的代理数
            "rate_limit_waits": 0,  # 限速等待次数
        }
    
    @classmethod
    def from_file(cls, filepath: str) -> "ProxyPool":
        """从文本文件加载代理列表（每行一个，格式：http://ip:port 或 socks5://ip:port）"""
        proxies = []
        try:
            ...
            print(f"    ⚠️ 代理文件 {filepath} 不存在，使用直连模式")
        return cls(proxies)
    
    @classmethod
    def from_free_sources(cls, max_proxies: int = 20) -> "ProxyPool":
        """
        从免费代理源自动抓取可用代理
        策略：以89ip为主（按省份抓取中国IP），验证后只保留可连通东财API的代理
        注意：免费代理质量参差不齐，成功率通常<5%，仅作为临时方案
        """
        proxies = []
        seen = set()

        # ── 89ip 按省份抓取（实测唯一可靠的免费源）──
        # 优先抓中国主要省份，提高中国IP比例
        regions = [
            ("全部", ""),
        ...
        for name, code in regions:
            try:
                url = f"https://www.89ip.cn/tqdl.html?api=1&num=50&port=&address={code}&isp="
                print(f"    📡 抓取89ip [{name}]: {url[:60]}...")
                ...
                found = re.findall(
                    ...
                    r'(?:25[0-5]|2[0-4]\d|1\d{2}|[1-9]?\d):\d{2,5}',
                    r.text
                )
                new_count = 0
                for p in found:
                    if p not in seen:
                        ...
                        new_count += 1
                print(f"       ↳ 获取 {len(found)} 个，新增 {new_count} 个（去重后）")
            except Exception as e:
                print(f"       ↳ 抓取失败: {type(e).__name__}")

        # ── 备用源：快代理（实测成功率低，作为补充）──
        try:
            print(f"    📡 抓取快代理备用源...")
            r = sess.get("https://www.kuaidaili.com/free/inha/1//", timeout=12, verify=False)
            if r.status_code == 200:
                found = re.findall(
                    r'(?:(?:25[0-5]|2[0-4]\d|1\d{2}|[1-9]?\d)\.){3}'
                    r'(?:25[0-5]|2[0-4]\d|1\d{2}|[1-9]?\d):\d{2,5}',
                ...
                for p in found:
                    if p not in seen:
                        ...
                        proxies.append(f"http://{p}")
                        new_count += 1
                print(f"       ↳ 获取 {len(found)} 个，新增 {new_count} 个")
        except Exception:
            pass

        if not proxies:
            print("    ⚠️ 未抓取到任何代理，将使用限速直连模式")
            return cls([])

        print(f"    📡 共抓取到 {len(proxies)} 个代理（去重后），正在并发验证可用性...")

        # ── 并发验证代理（实测需要验证更多才能找到可用的）──
        validate_count = min(len(proxies), max_proxies * 5)  # 验证5倍数量
        valid = cls._validate_proxies_concurrent(proxies[:validate_count], max_proxies)

        if valid:
            print(f"    ✅ 验证通过 {len(valid)}/{len(proxies)} 个代理")
        else:
            print(f"    ⚠️ 所有代理均不可用（免费代理成功率通常<5%），将使用限速直连模式")

        return cls(valid)

    @classmethod
    def _validate_proxies_concurrent(cls, proxies: list, max_keep: int = 20) -> list:
        """并发验证代理可用性（对东财API做快速测试）"""
        valid = []
        lock = threading.Lock()

        def _test_one(proxy):
            ok = cls._quick_test(proxy)
            with lock:
                if ok:
                    valid.append(proxy)
            return ok

        # 用 ThreadPoolExecutor 并发验证，最多15个线程加速
    ...
    @staticmethod
    def _quick_test(proxy: str, timeout: int = 8) -> bool:
        """快速测试代理是否能连通东财API（超时8秒，兼容慢速代理）"""
        try:
            r = requests.get(
    ...
    def available_count(self) -> int:
        """可用代理数量"""
        with self._lock:
            return len([p for p in self._proxies if self._bad.get(p, 0) < self._bad_threshold])
    
    def get(self) -> Optional[str]:
        """
        获取下一个可用代理
        返回 None 表示直连（无代理或所有代理都失效）
        在直连模式下，如果设置了 rate_limit，会自动限速等待
        """
        with self._lock:
            ...
            while tried < len(self._proxies):
                ...
                if self._bad.get(proxy, 0) < self._bad_threshold:
                    self._stats["proxy_requests"] += 1
                    return proxy
    ...
    def _wait_if_needed(self):
        ...
        if self._rate_limit > 0:
            ...
            if elapsed < self._rate_limit and self._last_request_time > 0:
                wait = self._rate_limit - elapsed
                self._stats["rate_limit_waits"] += 1
                time.sleep(wait)
            self._last_request_time = time.time()
    
    def mark_bad(self, proxy: Optional[str]):
        """标记代理请求失败"""
        with self._lock:
            if proxy is None:
                self._direct_fails += 1
                self._stats["direct_fail"] += 1
            else:
                self._bad[proxy] = self._bad.get(proxy, 0) + 1
                self._stats["proxy_fail"] += 1
                fails = self._bad[proxy]
                if fails >= self._bad_threshold:
                    print(f"    ⚠️ 代理 {proxy} 连续失败 {fails} 次，已剔除")
                    self._stats["proxies_removed"] += 1
    
    def mark_good(self, proxy: Optional[str]):
        """标记代理请求成功（重置失败计数）"""
        with self._lock:
            if proxy is None:
                ...
                self._stats["proxy_success"] += 1
                if proxy in self._bad:
                    self._bad[proxy] = 0
    
    def get_session_with_proxy(self, headers: dict = None) -> tuple:
        """
        获取一个带代理的 Session 和对应的 proxy URL
        返回: (session, proxy_url)
        proxy_url 为 None 表示直连
        """
        proxy = self.get()
        sess = requests.Session()
        if headers:
    ...
    def report(self) -> str:
        ...
        proxy_ok = s["proxy_success"]
        proxy_fail = s["proxy_fail"]
        direct_ok = s["direct_success"]
        direct_fail = s["direct_fail"]

        proxy_total = proxy_ok + proxy_fail
        direct_total = direct_ok + direct_fail
        ...
        if proxy_total > 0:
            proxy_rate = proxy_ok / proxy_total * 100
            lines.append(f"  代理请求: {proxy_total} (成功 {proxy_ok}, 失败 {proxy_fail}, 成功率 {proxy_rate:.0f}%)")
        if direct_total > 0:
            direct_rate = direct_ok / direct_total * 100
            lines.append(f"  直连请求: {direct_total} (成功 {direct_ok}, 失败 {direct_fail}, 成功率 {direct_rate:.0f}%)")

        lines.append(f"  被剔除代理: {s['proxies_removed']}")
        lines.append(f"  当前可用代理: {self.available_count}")
        
        if self._rate_limit > 0:
            lines.append(f"  限速模式: {self._rate_limit}s/请求 (触发等待 {s['rate_limit_waits']} 次)")

        return "\n".join(lines)


# 全局代理池实例（默认直连模式，可通过命令行参数或配置文件激活）
_proxy_pool = None

def _init_proxy_pool(proxies_file: str = None, use_free_proxies: bool = False, rate_limit: float = 0):
    """
    初始化全局代理池
    
    Args:
        proxies_file: 代理列表文件路径
        use_free_proxies: 是否自动抓取免费代理
        rate_limit: 直连模式下的请求间隔（秒），0=不限速
    """
    global _proxy_pool
    if proxies_file:
        _proxy_pool = ProxyPool.from_file(proxies_file)
        if rate_limit > 0:
            _proxy_pool._rate_limit = rate_limit
    elif use_free_proxies:
        _proxy_pool = ProxyPool.from_free_sources(max_proxies=15)
        # 免费代理成功率低，自动启用限速直连作为降级方案
        if _proxy_pool.available_count == 0:
            _proxy_pool._rate_limit = rate_limit or 1.5  # 默认1.5秒间隔
            print(f"    🐢 已启用限速直连模式（每次请求间隔 {_proxy_pool._rate_limit}s）")
        elif rate_limit > 0:
            _proxy_pool._rate_limit = rate_limit
    else:
        _proxy_pool = ProxyPool(rate_limit=rate_limit)  # 直连模式
    
    count = _proxy_pool.available_count
    if count > 0:
        print(f"    📡 代理池已激活：{count} 个可用代理")
    elif _proxy_pool._rate_limit > 0:
        print(f"    📡 代理池：限速直连模式（间隔 {_proxy_pool._rate_limit}s）")
    else:
        print(f"    📡 代理池：直连模式（无代理）")
...
# 每个域名独立限速
_limiter_sina = RateLimiter(0.2)      # 新浪：~5次/秒（提速：从0.3→0.2）
_limiter_tencent = RateLimiter(0.05)  # 腾讯：~20次/秒（不封IP，大幅提速：从0.1→0.05）

# ════════════════════════════════════════════════════
# 智能磁盘缓存（减少重复运行时间）
# ════════════════════════════════════════════════════

def _is_same_trading_day(file_mtime: float) -> bool:
    """判断文件修改时间是否在当天交易时段内（当天15:00之前=当日有效）"""
    file_dt = datetime.fromtimestamp(file_mtime)
    now_dt = datetime.now()
    # 同一天：不管什么时候都有效
    if file_dt.date() == now_dt.date():
        return True
    # 前一天：如果现在是早上9:00之前（盘前），前一天的收盘数据仍然有效
    if file_dt.date() == (now_dt.date() - timedelta(days=1)) and now_dt.hour < 9:
        return True
    return False

def _cache_load(cache_dir: str, prefix: str, max_age_seconds: float = 0,
                same_day_only: bool = True) -> Optional[dict]:
    """
    从缓存目录加载最新数据
    
    Args:
        cache_dir: 缓存目录路径
        prefix: 文件前缀（如 "quotes_", "sectors_"）
        max_age_seconds: 最大缓存年龄（秒），0=仅检查是否同日
        same_day_only: True=只接受当日的缓存，False=接受max_age_seconds内的
    """
    if not os.path.isdir(cache_dir):
        return None
    
    # 查找匹配前缀的最新文件
    pattern = os.path.join(cache_dir, f"{prefix}*.json")
    files = _glob.glob(pattern)
    if not files:
        # 尝试不带时间戳的文件名（如 all_codes_cache.json）
        alt_pattern = os.path.join(cache_dir, f"{prefix}.json")
    ...
    if same_day_only:
        if not _is_same_trading_day(mtime):
            return None
    ...
    try:
        ...
        age_str = f"{age/3600:.1f}h" if age > 3600 else f"{age/60:.0f}min" if age > 60 else f"{age:.0f}s"
        return data
    except Exception:
        return None

def _cache_load_klines(cache_dir: str, codes: list[str]) -> tuple[dict, list[str]]:
    """
    从磁盘缓存加载K线数据，返回(已缓存的K线, 需要重新获取的代码列表)
    
    K线缓存策略：当日文件有效，跳过已缓存股票的网络请求
    """
    ...
    return cached, need_fetch

def _cache_load_klines120(cache_dir: str, codes: list[str]) -> tuple[dict, list[str]]:
    """从磁盘缓存加载120minK线数据"""
    kline120_dir = os.path.join(cache_dir, "klines_120min")
    if not os.path.isdir(kline120_dir):
        return {}, codes
    ...
    for code in codes:
        fpath = os.path.join(kline120_dir, f"{code}.json")
        if os.path.exists(fpath):
            mtime = os.path.getmtime(fpath)
            if _is_same_trading_day(mtime):
                try:
...
def fetch_quotes_batch(codes: list[str], session: requests.Session = None) -> dict:
    ...
    for i in range(0, len(codes), batch_size):
        ...
        # 腾讯不封IP，RateLimiter(0.05s)已足够限速，无需额外抖动
        # （移除原 _jitter_delay，从每批0.15s额外延迟 → 0，63批累计省~9.5秒）
...
def fetch_kline_baidu(code: str, days: int = 300, session: requests.Session = None) -> list[dict]:
    """
    百度股市通K线 — 独有能力: 返回时自带 ma5/ma10/ma20 均价，不封IP。
    作为第4数据源（mootdx→腾讯→新浪→百度），进一步提升K线覆盖率。
    返回: [{"day", "open", "close", "high", "low", "volume", "amount", "ma5", "ma10", "ma20"}, ...]
    """
    if session is None:
        session = _build_session()
    ...
    try:
        r = safe_request(url, _limiter_sina, session, params=params, headers=headers, max_retries=2)
        if not r:
            return []
        d = r.json()
        result = d.get("Result", {})
        ...
        idx_time = key_map.get("time", 0)
        idx_open = key_map.get("open", 1)
        idx_close = key_map.get("close", 2)
        idx_high = key_map.get("high", 3)
        idx_low = key_map.get("low", 4)
        idx_vol = key_map.get("volume", 5)
        idx_amt = key_map.get("amount", 6)
        idx_ma5 = key_map.get("ma5avgprice", -1)
        idx_ma10 = key_map.get("ma10avgprice", -1)
        idx_ma20 = key_map.get("ma20avgprice", -1)
        ...
        for row in rows_raw:
            ...
            try:
                k = {
                    "day": fields[idx_time] if idx_time < len(fields) else "",
                    "open": fields[idx_open] if idx_open < len(fields) else "0",
                    "close": fields[idx_close] if idx_close < len(fields) else "0",
                    "high": fields[idx_high] if idx_high < len(fields) else "0",
                    "low": fields[idx_low] if idx_low < len(fields) else "0",
                    "volume": fields[idx_vol] if idx_vol < len(fields) else "0",
                    "amount": fields[idx_amt] if idx_amt < len(fields) else "0",
                }
                # 附加百度独有的均线数据
                if idx_ma5 >= 0 and idx_ma5 < len(fields):
                    k["ma5"] = fields[idx_ma5]
                if idx_ma10 >= 0 and idx_ma10 < len(fields):
                    k["ma10"] = fields[idx_ma10]
                if idx_ma20 >= 0 and idx_ma20 < len(fields):
                    k["ma20"] = fields[idx_ma20]
                klines.append(k)
...
def fetch_kline_batch(codes: list[str], days: int = 1000, max_workers: int = 16,
...
async def _async_fetch_kline_batch(codes: list[str], days: int = 1000,
    ...
    async with aiohttp.ClientSession(connector=sina_conn, timeout=timeout) as sina_sess, \
        ...
        async def _fetch_one(code):
            ...
            # 使用专用线程池限制并发数，避免mootdx连接数爆炸
            klines = []
            if _HAS_MOOTDX:
                try:
                    klines = await loop.run_in_executor(_mootdx_executor, fetch_kline_mootdx, code, min(days, 2400))
                except Exception:
                    klines = []
            ...
            if not klines:
                try:
                    baidu_klines = await _async_fetch_kline_baidu(tencent_sess, code, min(days, 300), semaphore)
                    if baidu_klines:
                        klines = baidu_klines
                except Exception:
                    pass
            ...
            if done_count % 200 == 0:
                ...
                fail_rate = fail_count / done_count * 100 if done_count > 0 else 0
                src = "mootdx(TCP)" if _HAS_MOOTDX else "HTTP(腾讯+新浪+百度)"
                print(f"    进度: {done_count}/{len(codes)} ({len(results)}有数据, 失败{fail_count} {fail_rate:.0f}%) | {speed:.0f}只/秒 | 剩余{eta:.0f}s | 源:{src}")
            return code, klines
    ...
    elapsed = time.time() - t_start
    fail_rate = fail_count / len(codes) * 100 if codes else 0
    print(f"    📊 异步K线完成: {len(results)}/{len(codes)} 有数据, 失败{fail_count} ({fail_rate:.0f}%), 耗时{elapsed:.1f}s ({len(codes)/elapsed:.0f}只/秒)")

    # 如果失败率>20%，自动重试失败股票（用新浪单线程降速重试）
    if fail_count > 0 and fail_rate > 20 and _HAS_AIOHTTP:
        failed_codes = [c for c in codes if c not in results]
        retry_count = min(len(failed_codes), 200)  # 最多重试200只
        if retry_count > 0:
            print(f"    ⚠️ 失败率{fail_rate:.0f}%过高，降速重试 {retry_count} 只...")
            await asyncio.sleep(3)  # 等待3秒让限流恢复
            retry_semaphore = asyncio.Semaphore(5)  # 降低并发到5
            retry_limiter = AdaptiveRateLimiter(base_rate=5, name="重试")
            retry_done = 0
            for code in failed_codes[:retry_count]:
                await retry_limiter.acquire()
                try:
                    klines = await _async_fetch_kline_sina(sina_sess, code, min(days, 1000), retry_semaphore)
                    if klines:
                        results[code] = klines
                        retry_limiter.on_success()
                    else:
                        retry_limiter.on_success()  # 空数据不算失败
                except Exception:
                    retry_limiter.on_429()
                retry_done += 1
                if retry_done % 50 == 0:
                    print(f"    重试进度: {retry_done}/{retry_count}")
            print(f"    ✅ 重试完成: {sum(1 for c in failed_codes[:retry_count] if c in results)}/{retry_count} 恢复")

    return results


async def _async_fetch_kline_baidu(session, code: str,
                                  days: int, semaphore) -> list[dict]:
    """异步百度K线（不封IP，自带MA5/10/20）"""
    url = "https://finance.pae.baidu.com/selfselect/getstockquotation"
    params = {
        "all": "1", "isIndex": "false", "isBk": "false", "isBlock": "false",
        "isFutures": "false", "isStock": "true", "newFormat": "1",
...
def fetch_all_stock_codes_em(session: requests.Session = None) -> tuple[list[str], dict]:
    ...
    for batch_idx in range(0, len(candidate_codes), batch_size):
        ...
        # 如果连续10批无数据，可能是API异常，提前终止
        if batch_num > 30 and len(valid_codes) == 0:
            print(f"    ⚠️ 连续{batch_num}批无数据，可能API异常，提前终止")
            break

        # 腾讯不封IP，RateLimiter(0.05s)已控制速度，移除额外抖动（63批省~5秒）
...
def fetch_dde_data_eastmoney(codes: list[str], session: requests.Session = None) -> dict:
    ...
    for code in codes:
        try:
            ...
            # 日K线格式：日期,主力净流入,超大单净流入,大单净流入,中单净流入,小单净流入,...
            # 取倒数第2行作为昨日数据（倒数第1行是当天盘中数据，可能不完整）
            if len(klines) >= 2:
                yesterday_line = klines[-2].split(",")
            else:
                yesterday_line = klines[-1].split(",")
...
def fetch_fund_flow_combined(codes: list[str], session: requests.Session = None,
    ...
    返回: (dde_dict, intraday_dict)
      ...
      intraday_dict: {code: {mid_buy_vol, tail_buy_vol, big_order_net, ...}}
    """
    if not codes:
        return {}, {}
    ...
    def _fetch_one(code):
        try:
            ...
            klines = data.get("data", {}).get("klines", [])
            if not klines:
                _source_tracker.mark("eastmoney_fflow", True)  # 接口可达但无数据
                return code, None, None

            # --- DDE数据：取倒数第2行（昨日完整交易日） ---
            # 东财 daykline 字段顺序：日期,主力净流入,超大单净流入,大单净流入,中单净流入,小单净流入
            dde = None
            if len(klines) >= 2:
                yd = klines[-2].split(",")
                if len(yd) >= 6:
...
def fetch_fund_flow_efinance(codes: list[str], max_workers: int = 8) -> tuple:
    """
    使用 efinance 底层接口（HTTP端口80，绕过东财HTTPS封禁）获取资金流向数据
    
    关键发现：东财封的是 HTTPS(443) 端口，但 HTTP(80) 的 fflow/kline 接口正常！
    接口：http://push2.eastmoney.com/api/qt/stock/fflow/kline/get?klt=1
    
    返回: (dde_dict, intraday_dict) — 格式与 fetch_fund_flow_combined 完全兼容
      dde_dict:     {code: {main_net_inflow, ...}}  — 当日主力累计（非昨日）
      intraday_dict: {code: {mid_buy_vol, tail_buy_vol, ...}}
    
    注意：此接口只返回当日分钟级数据，"昨日主力"会显示为当日累计值
    """
    if not codes:
        return {}, {}
    ...
    def _fetch_one(code):
        ...
        for attempt in range(3):
            try:
                time.sleep(random.uniform(0.03, 0.10))  # 轻量延迟（并发+QPS控制）
                ...
                if not data or data.get("data") is None:
                    # rc=100 表示该股票无分钟级资金流数据 → 尝试 efinance 库降级
                    if attempt == 0:  # 仅在首次遇到 rc=100 时尝试降级
                        fallback = _try_efinance_fallback(code)
                        if fallback:
                            return code, fallback[0], fallback[1]
                    return code, None, None
                ...
                return _parse_kline_result(code, klines)
    ...
    def _parse_kline_result(code, klines):
        """解析 klines 数据为 dde + intra 格式"""
        last_row = klines[-1].split(",")
        if len(last_row) < 6:
            return code, None, None
        
        main_net = float(last_row[1]) if last_row[1] else 0
        super_large = float(last_row[5]) if last_row[5] else 0
        ...
        dde = {
            ...
            "small_net": small_net,
            "dde_net_volume": main_net / 10000 if main_net else 0,
            "source": "efinance_http",
        }
        
        # 盘中大单流向聚合
        mid_large_total = 0; mid_super_total = 0
        tail_large_total = 0; tail_super_total = 0
        ...
        for kl in klines:
            ...
            lv = float(fields[4]) if fields[4] else 0
            sv = float(fields[5]) if fields[5] else 0
            
            if time_part >= "14:56":
                tail_large_total += abs(lv)
                tail_super_total += abs(sv)
            elif time_part >= "09:35":
                mid_large_total += abs(lv)
                mid_super_total += abs(sv)
        
        if large_net > 0:
            mid_buy_vol = int(mid_large_total / 10000)
            mid_sell_vol = 0
        else:
            mid_buy_vol = 0
            mid_sell_vol = int(mid_large_total / 10000)
        
        if super_large > 0:
            tail_buy_vol = int(tail_super_total / 10000)
            tail_sell_vol = 0
        else:
            tail_buy_vol = 0
            tail_sell_vol = int(tail_super_total / 10000)
        ...
        intra = {
            ...
            "tail_buy_vol": tail_buy_vol,
            "tail_sell_vol": tail_sell_vol,
            "big_order_net": large_net,
            "super_large_net": super_large,
            "main_net_inflow": main_net,
            "mid_buy_all": large_net > 0,
            "data_source": "efinance_http",
        }
        
        return code, dde, intra
    
    def _try_efinance_fallback(code):
        """
        当 HTTP 接口返回 rc=100（无数据）时，使用 efinance 库作为降级方案
        efinance 库内部可能使用不同的接口或缓存策略
        只能提供 DDE 级别数据（主力净流入），无法提供分钟级中段/尾段明细
        """
        try:
            ...
            # efinance 的列名可能变化，做兼容处理
            main_net = 0
            super_large = 0
            large_net = 0
            
            # 尝试多种可能的列名
            for col_name in ["主力净流入-净额", "主力净流入"]:
                if col_name in df.columns:
                    val = row.get(col_name, 0) if hasattr(row, 'get') else row[col_name]
                    main_net = float(val) if val else 0
                    break
            
            for col_name in ["超大单净流入-净额", "超大单净流入"]:
                if col_name in df.columns:
                    val = row.get(col_name, 0) if hasattr(row, 'get') else row[col_name]
                    super_large = float(val) if val else 0
                    break
            
            for col_name in ["大单净流入-净额", "大单净流入"]:
                if col_name in df.columns:
                    val = row.get(col_name, 0) if hasattr(row, 'get') else row[col_name]
                    large_net = float(val) if val else 0
                    break
            
            if main_net == 0 and super_large == 0 and large_net == 0:
                return None
            ...
            dde = {
                ...
                "small_net": 0,
                "dde_net_volume": main_net / 10000 if main_net else 0,
                "source": "efinance_fallback",
            }
            
            # efinance 降级模式：无分钟级数据，用 DDE 方向推断买卖量
            intra = {
                "mid_buy_vol": 0,
                "mid_sell_vol": 0,
                "tail_buy_vol": 0,
    ...
    print(f"    📦 获取资金流向数据(HTTP备用源): {len(codes)} 只...")
    
    failed_codes = []  # 记录失败的股票，用于第二轮重试
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if done % 15 == 0 or done == len(codes):
                print(f"    ⏳ 进度: {done}/{len(codes)}...")
            try:
                code, dde, intra = future.result(timeout=25)
                if dde:
                ...
                failed_codes.append(codes[done - 1] if done <= len(codes) else "?")
    
    # ── 第二轮：并发重试失败股票（替代原串行逐只重试）──
    if failed_codes:
        print(f"    ⚠️ 第一轮 {len(failed_codes)} 只未获取到数据，并发重试...")
        time.sleep(1)  # 等待1秒让限流恢复
        retry_workers = min(max_workers, len(failed_codes))
        with ThreadPoolExecutor(max_workers=retry_workers) as executor2:
            retry_futures = {executor2.submit(_fetch_one, c): c for c in failed_codes}
            for future2 in as_completed(retry_futures):
                try:
                    code, dde, intra = future2.result(timeout=20)
                    if dde:
                        dde_results[code] = dde
                        if intra:
                            intra_results[code] = intra
    ...
    ok_count = len(dde_results)
    total = len(codes)
    print(f"    ✅ 主力净流入: {ok_count}/{total} 只 | 盘中大单流向: {len(intra_results)} 只")
    if ok_count < total:
        missing = [c for c in codes if c not in dde_results]
        print(f"    ⚠️ 缺失: {', '.join(missing[:10])}{'...' if len(missing)>10 else ''}")
    return dde_results, intra_results
...
def _check_eastmoney_https() -> bool:
    ...
    try:
        ...
        _source_tracker.mark("eastmoney_https", False)
        return False
...
def _check_eastmoney_http() -> bool:
    ...
    try:
        ...
        _source_tracker.mark("eastmoney_http", False)
        return False
    ...
    if https_ok:
        ...
        for code in codes:
            try:
                ...
                if not klines or len(klines) < 2:
                    continue
                # DDE数据取倒数第2行（昨日完整交易日），字段顺序：主力净流入,超大单,大单,中单,小单
                yd_row = klines[-2].split(",") if len(klines) >= 2 else klines[-1].split(",")
                if len(yd_row) >= 6:
                    main_net = float(yd_row[1]) if yd_row[1] else 0
                    dde[code] = {
                        "main_net_inflow": main_net,
                        "super_large_net": float(yd_row[2]) if yd_row[2] else 0,
                        "large_net": float(yd_row[3]) if yd_row[3] else 0,
                        "medium_net": float(yd_row[4]) if yd_row[4] else 0,
                        "small_net": float(yd_row[5]) if yd_row[5] else 0,
                        "dde_net_volume": main_net / 10000 if main_net else 0,
                        "source": "eastmoney_http",
                    }
                    # 盘中大单流向取最后一行（当日数据）
                    last_row = klines[-1].split(",")
                    large_flow = float(last_row[4]) if len(last_row) > 4 and last_row[4] else 0
                    super_large_flow = float(last_row[5]) if len(last_row) > 5 and last_row[5] else 0
                    if large_flow > 0:
                        mid_buy_vol = int(abs(large_flow) / 10000)
                        mid_sell_vol = 0
                        mid_buy_all = True
        ...
        print(" ❌ 东财push2全封（HTTPS+HTTP），尝试备用数据源...")
        print(f"    ⚠️ 数据源状态: {_source_tracker.summary()}")
        dde, intra = _fetch_fund_flow_fallback(codes)

    # 记录失败股票到负向缓存（线程安全），缩短到60秒
    failed = [c for c in codes if c not in dde]
    if failed:
        with fail_lock:
            expire = time.time() + 60  # 从300秒缩短到60秒，快速重试
            for c in failed:
                fail_cache[c] = expire
        print(f"    📝 {len(failed)} 只记录负向缓存（60秒后重试）")

    return dde, intra


def _fetch_fund_flow_fallback(codes: list[str]) -> tuple:
    """
    备用数据源降级链（当东财push2被封时）：
    1. akshare (data.eastmoney.com) — 批量排名接口
    2. efinance (HTTP 80端口) — 逐只获取
    3. push2his.eastmoney.com — 备用域名
    """
    dde, intra = {}, {}

    # ── 方案1：akshare 批量排名（一次请求拿全市场主力净流入排名）──
    try:
        import akshare as ak
        print("    📦 [备用1] 尝试 akshare 批量资金流向排名...")
        ...
        if df_rank is not None and not df_rank.empty:
            ...
            if code_col:
                # 找主力净流入列
                inflow_col = None
                for col in ["主力净流入-净额", "主力净流入", "今日主力净流入-净额"]:
                    if col in df_rank.columns:
                        inflow_col = col
                        break
                if inflow_col:
                    for _, row in df_rank.iterrows():
                        ...
                        if c in codes:
                            main_net = float(row[inflow_col]) if row[inflow_col] else 0
                            dde[c] = {
                                "main_net_inflow": main_net,
                                "super_large_net": 0,
                                "large_net": 0,
                    ...
                    print(f"    ✅ [备用1] akshare 匹配到 {len(dde)}/{len(codes)} 只")
                else:
                    print("    ⚠️ [备用1] akshare 未找到主力净流入列")
            else:
                print("    ⚠️ [备用1] akshare 未找到代码列")
        else:
            print("    ⚠️ [备用1] akshare 返回空数据")
    except ImportError:
        print("    ⏭️ [备用1] akshare 未安装，跳过")
    except Exception as e:
        print(f"    ⚠️ [备用1] akshare 异常: {type(e).__name__}: {e}")

    # 对未获取到的股票尝试方案2
    missing = [c for c in codes if c not in dde]
    if not missing:
        return dde, intra

    # ── 方案2：efinance 库（可能走不同接口/端口）──
    try:
        import efinance as ef
        print(f"    📦 [备用2] 尝试 efinance 库获取 {len(missing)} 只...")
        ok = 0
        for code in missing:
            try:
                ...
                if df is not None and not df.empty:
                    ...
                    for col in ["主力净流入-净额", "主力净流入"]:
                        if col in df.columns:
                            val = row.get(col, 0) if hasattr(row, 'get') else row[col]
                            main_net = float(val) if val else 0
                            break
                    if main_net != 0:
                        dde[code] = {
                ...
                continue
        print(f"    ✅ [备用2] efinance 获取到 {ok}/{len(missing)} 只")
    except ImportError:
        print("    ⏭️ [备用2] efinance 未安装，跳过")
    except Exception as e:
        print(f"    ⚠️ [备用2] efinance 异常: {type(e).__name__}: {e}")
    ...
    # ── 方案3：push2his.eastmoney.com 备用域名 ──
    print(f"    📦 [备用3] 尝试 push2his.eastmoney.com 获取 {len(missing)} 只...")
    ok = 0
    for code in missing:
        try:
            market = "1" if code.startswith("6") else "0"
            ...
            continue
    print(f"    ✅ [备用3] push2his 获取到 {ok}/{len(missing)} 只")

    return dde, intra


def fetch_dde_data_batch_legacy(codes: list[str], session: requests.Session = None,
...
def scrape_all(output_dir: str = "stock_data", target_codes: list[str] = None, no_cache: bool = False):
    """
    完整抓取流程（缓存加速版）：
    1. 智能缓存：当日已抓取的数据直接跳过
    2. 腾讯行情 + 东财行业分类（不封IP）
    3. 板块从股票数据直接构建（零额外API请求）
    4. K线：只抓取缺失的股票，合并缓存数据
    """
    ...
    try:
        ...
        if target_codes:
            ...
            cached_codes = _cache_load(output_dir, "all_codes_", max_age_seconds=86400, same_day_only=False) if not no_cache else None
            if cached_codes and len(cached_codes) > 100:
                age = time.time() - os.path.getmtime(
                    max(_glob.glob(os.path.join(output_dir, "all_codes_*.json")),
                        key=lambda f: os.path.getmtime(f)))
                print(f"\n📋 [2/5] 使用缓存的股票代码 ({len(cached_codes)} 只, {age/3600:.1f}小时前)")
        ...
        # ── 2. 腾讯行情（缓存5分钟）──
        cached_quotes = _cache_load(output_dir, "quotes_", max_age_seconds=300, same_day_only=False) if not no_cache else None
        if cached_quotes and len(cached_quotes) > 100:
            age = time.time() - os.path.getmtime(
                max(_glob.glob(os.path.join(output_dir, "quotes_*.json")),
                    key=lambda f: os.path.getmtime(f)))
            print(f"\n📊 [3/5] 使用缓存行情 ({len(cached_quotes)} 只, {age/60:.1f}分钟前)")
        ...
        # ── 3. 板块（缓存同日有效）──
        cached_sectors = _cache_load(output_dir, "sectors_", same_day_only=True) if not no_cache else None
        if cached_sectors:
            print(f"\n📊 [4/5] 使用缓存板块数据 ({len(cached_sectors)} 个板块)")
        ...
        cached_klines, need_fetch_codes = _cache_load_klines(output_dir, kline_codes) if not no_cache else ({}, kline_codes)
        cached_klines120, need_fetch_codes120 = _cache_load_klines120(output_dir, kline_codes) if not no_cache else ({}, kline_codes)

        if cached_klines:
            print(f"\n📊 [5/5] K线缓存命中: {len(cached_klines)}/{len(kline_codes)} 只")
        
        if need_fetch_codes:
            print(f"    需要抓取: {len(need_fetch_codes)} 只, 并发80...")
            fresh_klines, fresh_klines_120 = fetch_all_klines_async(need_fetch_codes, days=1000, concurrency=80)
            klines = {**cached_klines, **fresh_klines}
            klines_120 = {**cached_klines120, **(fresh_klines_120 or {})}
        else:
            klines = cached_klines
            klines_120 = cached_klines120
            print(f"    ✅ 所有K线已缓存，跳过抓取！")
        ...
        for code, data in klines.items():
            fpath = os.path.join(kline_dir, f"{code}.json")
            if code not in cached_klines or not os.path.exists(fpath):
                _save_json(data, fpath)
        ...
        for code, data in klines_120.items():
            fpath = os.path.join(kline120_dir, f"{code}.json")
            if code not in cached_klines120 or not os.path.exists(fpath):
                _save_json(data, fpath)
...
def close_auction_monitor(codes: list[str] = None, poll_interval: int = 10,
    ...
    # ---- 获取DDE大单数据 + 盘中实时大单流向（合并单次API调用）----
    print("📊 获取DDE大单数据 + 盘中实时大单流向...")
    final_codes = [c["code"] for c in final]
    
    # 合并调用：自动选择数据源（HTTPS优先，被封则降级HTTP）
    dde_data, intraday_flow = fetch_fund_flow_auto(final_codes)
    ...
    # ---- 按频次优先排序（频次→DDE净量→涨幅），保留前10 ----
    def _dde_sort_key(x):
        dde_net = x.get("dde_net_volume", 0)
        if dde_net > 5: return 5
        elif dde_net > 3: return 4
        elif dde_net > 1: return 3
        elif dde_net < -3: return 1
        elif dde_net < -1: return 2
        else: return 2.5

    final.sort(key=lambda x: (
        -x.get("frequency", 0),       # 频次（主排序）
        -_dde_sort_key(x),            # DDE净量分级
    ...
    # ---- 输出结果（按图中格式显示）----
    # 检测数据源，决定表头
    _any_efinance = any(s.get("dde_source") == "efinance_http" for s in final)
    _main_force_label = "当日主力" if _any_efinance else "昨日主力"
    
    print(f"\n{'═'*120}")
    print(f"  【模式3_追涨模式】竞价选股汇总（按频次优先）V12")
    if _any_efinance:
        print(f"  ⚠️ 东财HTTPS被封，使用HTTP备用源（显示为当日累计主力，非昨日数据）")
    print(f"{'═'*120}")
    
    # 表头
    print(f"  {'代码':<8} {'净流入':>10} {'名称':<18} {'频次-爆量比':>12} {_main_force_label:>8} {'筹码判断':<16} {'尾段竞价':<40} {'09:25':>7} {'09:26':>7} {'撮合量':>8} {'竞昨比':>7} {'剩余率':>7}")
    print(f"  {'─'*116}")
...
def _build_tail_description(s: dict) -> str:
    """
    构建尾段竞价详细描述（匹配目标V12格式）
    格式：谨慎/回避 中段:X买/Y卖(状态)->尾段:A买/B卖(状态) [价格|*大卖单]
    示例：谨慎 中段:17买/8卖(强势)->尾段:47买/21卖(分歧) 11.62
         回避 中段:0买/2515卖(出货)->尾段:0买/4121卖(出货) *大卖单
    """
    ...
    # ---- 构建中段描述（始终输出） ----
    mid_info = f"中段:{mid_buy_vol}买/{mid_sell_vol}卖({_state(mid_buy_vol, mid_sell_vol)})"

    # ---- 构建尾段描述（始终输出） ----
    tail_info = f"尾段:{tail_buy_vol}买/{tail_sell_vol}卖({_state(tail_buy_vol, tail_sell_vol)})"

    # ---- 后缀：*大卖单 或 价格 ----
    suffix = ""
    if big_order_net < -50000:
        suffix = "*大卖单"
    else:
        auction_price = s.get("auction_price", 0) or s.get("price", 0)
        if auction_price > 0:
            suffix = f"{auction_price:.2f}"

    # ---- 组合最终描述 ----
    result = f"{attitude} {mid_info}->{tail_info}"
    if suffix:
        result += f" {suffix}"
...
{idx_cells}
    ...
    for i, s in enumerate(stocks, 1):
        ...
        # 筹码判断（V12格式：不显示排名）
        verdict = s.get("verdict", "正常")
        verdict_html = f'{html_module.escape(verdict)}'
...
<div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap">{ml_cells}</div>
    ...
    # 检测数据源，决定表头
    _any_efinance_html = any(s.get("dde_source") == "efinance_http" for s in stocks)
    _main_force_label_html = "当日主力" if _any_efinance_html else "昨日主力"
    _source_note_html = '<div style="text-align:center;color:#f0883e;font-size:11px;margin:4px 0">⚠️ 东财HTTPS被封，使用HTTP备用源（显示为当日累计主力）</div>' if _any_efinance_html else ""

    html = f"""<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>竞价选股汇总（按频次优先）</title>
<style>
...
<div class="hd"><h1>【模式3_追涨模式】竞价选股汇总（按频次优先）V12</h1><div class="t">更新时间: {now}</div></div>{_source_note_html}
{idx_html}
{main_line_html}
{leader_html}
{sector_html}
...
<table>
<thead><tr>
<th>代码</th><th>净流入</th><th>名称</th><th>频次-爆量比</th><th>{_main_force_label_html}</th><th>筹码判断</th><th>尾段竞价</th>
...
def run_from_data_dir(data_dir: str, html_path: str = None, quiet: bool = False) -> list[dict]:
    ...
    # ---- 9.5. 获取DDE大单数据 + 盘中实时大单流向（合并单次API调用）----
    print("📊 获取DDE大单数据 + 盘中实时大单流向...")
    final_codes = [c["code"] for c in final]
    session = _build_session()
    ...
    if not quiet:
        # 检测数据源
        _any_efinance_3 = any(s.get("dde_source") == "efinance_http" for s in final)
        _main_force_label_3 = "当日主力" if _any_efinance_3 else "昨日主力"
        ...
        # 表头
        print(f"  {'代码':<8} {'净流入':>10} {'名称':<18} {'频次-爆量比':>12} {_main_force_label_3:>8} {'筹码判断':<16} {'尾段竞价':<40} {'09:25':>7} {'09:26':>7} {'撮合量':>8} {'竞昨比':>7} {'剩余率':>7}")
        print(f"  {'─'*116}")
...
def run_oneclick(no_cache: bool = False):
    inputs = sys.argv[1:]
    session = _build_session()
    ...
    print(f"\n{'═'*50}")
    print(f"  NYLO — A股数据抓取 + 分析（缓存加速版）")
    if no_cache:
        print(f"  ⚠️ --no-cache 模式：强制刷新所有数据")
    print(f"  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'═'*50}\n")

    try:
        # ══════════════════════════════════════════
        # 1. 大盘指数（无缓存，必须实时获取，耗时~2秒）
        # ══════════════════════════════════════════
        print("📊 获取大盘...")
        indices = fetch_indices(session)
        for idx in indices:
        ...
        # ══════════════════════════════════════════
        # 2. 行情 + 板块 — 智能缓存（no_cache时跳过缓存）
        # ══════════════════════════════════════════
        use_cache = not no_cache
        
        if codes:
            # 指定代码模式：行情缓存5分钟
            cached_quotes = _cache_load(data_dir, "quotes_", max_age_seconds=300, same_day_only=False) if use_cache else None
            if cached_quotes and len(cached_quotes) >= len(codes):
                # 检查缓存中是否包含所有指定代码
                cached_codes_in_quotes = set(cached_quotes.keys())
                if all(c in cached_codes_in_quotes for c in codes):
                    quotes = {c: cached_quotes[c] for c in codes if c in cached_quotes}
                    age = time.time() - os.path.getmtime(_glob.glob(os.path.join(data_dir, "quotes_*.json"))[-1])
                    print(f"\n📊 使用缓存行情 ({len(quotes)} 只, {age:.0f}秒前)")
                    sectors = {}
                else:
                    print(f"\n📊 获取行情 ({len(codes)} 只)...")
            ...
            # 全市场模式：行情缓存5分钟
            cached_quotes = _cache_load(data_dir, "quotes_", max_age_seconds=300, same_day_only=False) if use_cache else None
            cached_codes = _cache_load(data_dir, "all_codes_", max_age_seconds=86400, same_day_only=False) if use_cache else None
            
            codes_loaded_from_cache = False
            
            if cached_codes and len(cached_codes) > 100:
                age_codes = time.time() - os.path.getmtime(
                    max(_glob.glob(os.path.join(data_dir, "all_codes_*.json")), 
                        key=lambda f: os.path.getmtime(f)))
                print(f"\n📊 使用缓存的股票代码 ({len(cached_codes)} 只, {age_codes/3600:.1f}小时前)")
                codes = cached_codes
                codes_loaded_from_cache = True
            else:
                ...
                if len(all_codes) < 100:
                    ...
                    stock_info = {}
                codes = all_codes
                _save_json(codes, f"{data_dir}/all_codes_{timestamp}.json")

            if cached_quotes and len(cached_quotes) > 100:
                age_q = time.time() - os.path.getmtime(
                    max(_glob.glob(os.path.join(data_dir, "quotes_*.json")),
                        key=lambda f: os.path.getmtime(f)))
                if age_q < 300:  # 5分钟内
                    print(f"📊 使用缓存行情 ({len(cached_quotes)} 只, {age_q/60:.1f}分钟前)")
                    quotes = cached_quotes
                else:
                    print(f"\n📊 腾讯批量行情 ({len(codes)} 只)...")
                    quotes = fetch_quotes_batch(codes, session)
            ...
            # 板块缓存（同日有效）
            cached_sectors = _cache_load(data_dir, "sectors_", same_day_only=True) if use_cache else None
            if cached_sectors:
                print(f"📊 使用缓存板块数据 ({len(cached_sectors)} 个板块)")
                sectors = cached_sectors
            elif not codes_loaded_from_cache:
        ...
        # 当日已缓存的K线直接跳过，只抓取缺失的
        # ══════════════════════════════════════════
        cached_klines, need_fetch_codes = _cache_load_klines(data_dir, codes) if use_cache else ({}, codes)
        cached_klines120, need_fetch_codes120 = _cache_load_klines120(data_dir, codes) if use_cache else ({}, codes)
        
        if cached_klines:
            print(f"\n📊 K线缓存命中: {len(cached_klines)}/{len(codes)} 只（跳过已缓存）")
        
        if need_fetch_codes:
            print(f"📊 需要抓取K线: {len(need_fetch_codes)} 只, 并发80...")
            fresh_klines, fresh_klines_120 = fetch_all_klines_async(need_fetch_codes, days=1000, concurrency=80)
            ...
            for code in need_fetch_codes:
                if code not in need_fetch_codes120 and code in cached_klines120:
                    # 已有缓存但需要更新日K线，120min可能仍有效
                    pass
            fresh_120 = fresh_klines_120 if fresh_klines else {}
            klines_120 = {**cached_klines120, **fresh_120}
        else:
            # 全部缓存命中，跳过K线抓取！
            klines = cached_klines
            klines_120 = cached_klines120
            print(f"  ✅ 所有K线已缓存，跳过抓取（省30-300秒！）")
        ...
        print(f"\n{'═'*50}")
        print(f"  🚀 开始分析...")
        elapsed_fetch = time.time() - t_total_start
        print(f"  数据获取耗时: {elapsed_fetch:.0f}秒")
        print(f"{'═'*50}\n")
        ...
        print(f"\n📄 报告: {html_out}")
        print(f"⏱️ 总耗时: {elapsed_total:.0f}秒 (数据获取{elapsed_fetch:.0f}s)")
        print(f"📡 数据源状态: {_source_tracker.summary()}")
...
示例:
  ...
  python3 NY3.0.py --data-dir stock_data              # 从本地数据离线分析
  python3 NY3.0.py --free-proxy                       # 自动抓取免费代理（临时方案）
  python3 NY3.0.py --rate-limit 1.5                   # 限速直连（每次请求间隔1.5秒）
  python3 NY3.0.py --free-proxy --rate-limit 2.0      # 免费代理+限速降级
  python3 NY3.0.py --proxy-file proxies.txt            # 从文件加载代理列表
  python3 NY3.0.py --no-cache                          # 强制刷新，忽略磁盘缓存
        """,
    )
    ...
    parser.add_argument("--proxy-file", metavar="FILE",
                        help="代理列表文件（每行一个 http://ip:port），解决东财封IP问题")
    parser.add_argument("--free-proxy", action="store_true",
                        help="自动抓取免费代理（质量参差不齐，仅作临时方案）")
    parser.add_argument("--rate-limit", type=float, default=0, metavar="SECONDS",
                        help="直连模式请求间隔秒数（避免被封IP，推荐1.0~2.0）")
    parser.add_argument("--no-cache", action="store_true",
                        help="强制刷新所有数据，忽略磁盘缓存")

    args = parser.parse_args()

    # ---- 初始化代理池 ----
    _init_proxy_pool(proxies_file=args.proxy_file, use_free_proxies=args.free_proxy,
    ...
    try:
        ...
        # 无论是否出错，都输出代理池报告
        if _proxy_pool and _proxy_pool._stats["total_requests"] > 0:
            print(f"\n{_proxy_pool.report()}")

def _main_inner(args):
    """main() 的实际逻辑，被 try/finally 包裹以输出代理报告"""
    ...
    if not args.inputs and not args.data_dir and not args.search and not args.test and not args.codes:
        ...
        if choice == "1":
            run_oneclick(no_cache=args.no_cache)
            return
        elif choice == "2":
    ...
    # ---- 纯抓取模式（--codes 或 全市场）----
    try:
        scrape_all(output_dir=args.output, target_codes=args.codes, no_cache=args.no_cache)
    except Exception as e:
        print(f"\n⚠ 抓取出错: {e}")
        print("  继续尝试分析已有数据...\n")
