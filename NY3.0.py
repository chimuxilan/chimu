...
def fetch_stock_details(codes: list[str], session: requests.Session = None,
    ...
    quotes_ref: 可选，腾讯行情数据，用于交叉验证东财返回的价格是否正确
    返回: {code: {market_cap_yi, volume_ratio, turnover, amount, volume}}
    """
    if session is None:
        session = _build_session()
            ...
            if m:
                ...
                for item in data.get("data", {}).get("diff", []):
                    ...
                    results[code] = {
                        ...
                        "amount": round(float(item.get("f6", 0) or 0) / 1e4, 2),  # 元→万元
...
def run_from_data_dir(data_dir: str, html_path: str = None, quiet: bool = False) -> list[dict]:
    ...
    details = {}
    if detail_files:
        with open(detail_files[0], "r", encoding="utf-8") as f:
            details = json.load(f)
