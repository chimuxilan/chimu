...
# 不同域名用不同限速器，互不影响
_rate_sina = _RateLimiter(min_interval=0.25)    # 新浪接口：每秒最多4次
_rate_tencent = _RateLimiter(min_interval=0.15)  # 腾讯接口：每秒最多6次
_rate_eastmoney = _RateLimiter(min_interval=0.2) # 东财接口：每秒最多5次


def _rate_limited_request(url: str, limiter: _RateLimiter, method: str = "GET",
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if attempt < max_retries - 1:
                _time.sleep(1.0)
    raise requests.exceptions.RequestException(f"请求失败({max_retries}次重试): {last_err}")
...
def _wait_for_auction_data(max_wait_sec: int = 30, poll_interval: int = 5) -> bool:
    ...
    while elapsed < max_wait_sec:
        try:
            r = _rate_limited_request(f"https://qt.gtimg.cn/q={probe}", _rate_tencent, headers=HEADERS, timeout=10)
            r.encoding = "gbk"
...
def fetch_quotes(codes: list[str]) -> dict:
    ...
    try:
        r = _rate_limited_request(f"https://qt.gtimg.cn/q={symbols}", _rate_tencent, headers=HEADERS, timeout=15)
        r.encoding = "gbk"
    except Exception as e:
        print(f"  ⚠ 行情请求失败: {e}")
        return {}
    ...
    for attempt in range(3):
        try:
            r = _rate_limited_request(
                ...
                _rate_sina,
                params={"symbol": sina_sym, "scale": "240", "ma": "no", "datalen": days},
                headers={"User-Agent": "Mozilla/5.0", "Referer": "https://finance.sina.com.cn/"},
                timeout=15, max_retries=2,
            )
            if r.text and r.text.strip() and r.text.strip() != "null":
    ...
    for attempt in range(2):
        try:
            ...
            r = _rate_limited_request(
                ...
                headers=HEADERS, timeout=15, max_retries=2,
            )
...
def fetch_market_indices() -> list[dict]:
    ...
    symbols = ",".join(sym for _, sym in index_map)
    name_lookup = {sym: name for name, sym in index_map}
    results = []
    try:
        r = _rate_limited_request(f"https://qt.gtimg.cn/q={symbols}", _rate_tencent, headers=HEADERS, timeout=10)
        r.encoding = "gbk"
        for line in r.text.strip().split("\n"):
            m = re.search(r'v_(\w+)="(.+)"', line)
            if not m:
            ...
            if len(fields) < 44:
                continue
            name = name_lookup.get(symbol, fields[1])
            results.append({
                "name": name,
                "code": symbol,
...
def _fetch_em_kline_batch(codes: list[str], days: int = 1000) -> dict:
    """
    通过东方财富 push2 批量接口获取日K线（单次请求可覆盖数百只股票，大幅减少请求次数）
    返回: {code: [kline_data]} 格式与 fetch_hist 一致
    """
    EM_HEADERS = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
    ...
    result = {}
    # 东方财富 kline 接口支持 secids 批量，但返回数据是单只的，需要用 fl=secid 分组请求
    # 改用 stock 分批，每批50只，利用并发减少总耗时
    batch_size = 50
        ...
        for code in batch_codes:
            ...
            try:
                r = _rate_limited_request(
                    ...
                    headers=EM_HEADERS, timeout=15, max_retries=2,
                )
                try:
                    resp_json = r.json()
                ...
                if klines_raw:
                    ...
                    for line in klines_raw:
                        parts = line.split(",")
                        if len(parts) >= 6:
                            # 东财K线成交量单位为"股"，统一转换为"手"（÷100）与新浪/腾讯保持一致
                            vol_shares = float(parts[5]) if parts[5] else 0
                            vol_lots = int(vol_shares / 100) if vol_shares > 0 else 0
                            klines.append({
                                "day": parts[0],
                                "open": parts[1],
                                "close": parts[2],
...
def _fetch_kline_concurrent(codes: list[str], days: int = 5) -> dict:
    """
    获取多只股票的日K线。
    优先用东方财富批量接口（单次请求可覆盖数百只），失败的股票再用新浪逐只补救。
    返回: {code: [kline_data]}
    """
    # 东方财富批量获取
    result = _fetch_em_kline_batch(codes, days=days)

    # 未获取到的股票，用新浪逐只补救（限速+低并发）
    missing = [c for c in codes if c not in result]
    if missing:
        print(f"  ⚠ 东方财富K线缺失 {len(missing)} 只，新浪补救中...")
        def _fetch_one(code):
            try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            futures = [executor.submit(_fetch_one, c) for c in missing]
            for future in concurrent.futures.as_completed(futures):
...
# 120分钟MACD缓存（避免同一只股票重复请求）
_macd_120min_cache: dict = {}
...
def _check_macd_120min_up(code: str, cached_klines: list[dict] = None) -> bool:
    ...
    try:
        ...
        if day_klines and len(day_klines) >= _min_len:
    ...
    # 方案3: 标准EMA(12,26,9)做最后尝试（需35根）
    try:
        fallback_klines = day_klines if 'day_klines' in dir() and day_klines else (cached_klines or fetch_hist(code, days=60))
        if fallback_klines and len(fallback_klines) >= 35:
            closes = [float(k.get("close", 0)) for k in fallback_klines if float(k.get("close", 0)) > 0]
            if len(closes) >= 35:
                macd_line, _, _ = _compute_macd(closes, fast=12, slow=26, signal=9)
                if len(macd_line) >= 2:
                    result = macd_line[-1] > macd_line[-2] and macd_line[-1] > 0
    except Exception:
        pass
...
def _fetch_em_stock_details(codes: list[str]) -> dict:
    ...
    for i in range(0, len(secids), batch_size):
        ...
        for attempt in range(3):
            try:
                r = _rate_limited_request(
                    ...
                    headers=EM_HEADERS, timeout=20, max_retries=2,
                )
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    if not code or len(code) != 6:
                        continue
                    mktcap_yuan = float(item.get("f21", 0) or 0)  # f21 = 流通市值（元）
                    if mktcap_yuan <= 0:
                        mktcap_yuan = float(item.get("f20", 0) or 0)  # f20 = 总市值，作为回退
            ...
            print(f"  ⚠ 东方财富批量接口批次{i // batch_size + 1}解析失败: {e}")
...
def pool_base_post_kline(code: str, price: float, klines: list[dict],
    ...
    # 定位"昨日"K线：取倒数第1根（如果其日期是今天则取倒数第2根）
    # 节假日/周末时 klines[-1] 可能不是今天，而是上一个交易日
    _today_str = datetime.now().strftime("%Y-%m-%d")
    _last_day = str(klines[-1].get("day", ""))[:10]
    if _last_day == _today_str:
        # 最后一根是今天（盘中运行），昨日 = klines[-2]，前日 = klines[-3]
        _offset = 0  # klines[-1] = today
    else:
        # 最后一根不是今天（盘前/竞价/节假日），最后一根就是"昨日"
        _offset = 1  # klines[-1] = yesterday

    yesterday_kline = klines[-(1 + _offset)] if len(klines) >= 1 + _offset else None
    dby_kline = klines[-(2 + _offset)] if len(klines) >= 2 + _offset else None
    ...
    y_amount = float(yesterday_kline.get("amount", 0))  # 昨日成交额（需K线数据包含amount字段）

    # 9. 昨日成交金额 > 上次涨停日成交金额
    # 找最近一次涨停日（排除昨日），比较昨额与涨停额
    # 注意：部分K线数据源（新浪/腾讯日K）不含amount字段，此条件自动跳过
    limit_pct = 10  # 主板
    threshold = limit_pct * 0.95
    last_limit_amount = 0
    if y_amount > 0:  # 只在有成交额数据时才执行此检查
    ...
    # 10. 7天涨幅 < 34.99%
    if len(klines) >= 8 + _offset:
        close_7d_ago = float(klines[-(8 + _offset)].get("close", 0))
        if close_7d_ago > 0:
    ...
    if dby_kline:
        ...
        if dby_close > 0:
            ...
            if y_chg >= threshold:
                ...
                if len(klines) >= 4 + _offset:
                    dby_prev = float(klines[-(4 + _offset)].get("close", 0))
                    if dby_prev > 0:
    ...
    if not _before_market_open and _offset == 0:
        # 仅在盘中且有今日K线时才检查
...
def pool_volume_price(c: dict, tc: dict, em: dict, klines: list = None) -> tuple[bool, str]:
    ...
    if klines and len(klines) >= 3:
        try:
            # 判断最新K线是否为今天（盘前/非交易日时最新K线=昨日）
            _today_s = datetime.now().strftime("%Y-%m-%d")
            _latest_day = str(klines[-1].get("day", ""))[:10]
            _off = 0 if _latest_day == _today_s else 1
            dby_close = float(klines[-(3 - _off)].get("close", 0))
            y_close = float(klines[-(2 - _off)].get("close", 0))
            if dby_close > 0:
                y_chg = (y_close - dby_close) / dby_close * 100
                if y_chg >= 9.5:  # 主板涨停阈值
                    return False, "前一日涨停"
    ...
    # 9. 量比 > 5
    # 估算竞价时段平均量：用近5日成交量均值 × 竞量系数（竞价时段约占日均量的0.3%~0.5%）
    if klines and len(klines) >= 3:
        recent_vols = []
        for k in klines[-5:-1]:  # 排除今日，取前4个交易日
            try:
                v = int(float(k.get("volume", 0)))
                if v > 0:
                    recent_vols.append(v)
            except (ValueError, TypeError):
                pass
        avg_daily_vol = np.mean(recent_vols) if recent_vols else yesterday_vol_lots
    else:
        avg_daily_vol = yesterday_vol_lots
    # 竞价时段平均量 ≈ 日均量 × 0.004（经验系数，基于竞价10分钟 vs 全天240分钟，
    # 但竞价时段活跃度通常是全天的3~5倍，综合系数约 10/240 × 3.5 ≈ 0.15，再取保守值）
    est_auction_avg = avg_daily_vol * 0.004 if avg_daily_vol > 0 else 1
...
def pool_trend(c: dict, klines: list[dict], tc: dict = None, em: dict = None) -> tuple[bool, str]:
    ...
    if yesterday_vol_lots > 0 and auction_vol > 0:
        # 用近5日均量估算竞价平均量（与 pool_volume_price 保持一致）
        if klines and len(klines) >= 3:
            recent_vols = []
            for k in klines[-5:-1]:
                try:
...
def _fetch_sectors_with_stocks() -> dict:
    """
    通过东方财富 push2 接口获取行业板块列表及各板块下的股票（带限速）
...
def screen_mainboard_strategy() -> list[dict]:
    ...
    print("📊 获取腾讯竞价详细数据...")
    tencent_map = {}
    batch_size = 80  # 腾讯接口支持大批量，80只一次请求
    for i in range(0, len(all_candidates), batch_size):
        ...
        try:
            r = _rate_limited_request(
                ...
                _rate_tencent, headers=HEADERS, timeout=15,
            )
            r.encoding = "gbk"
            for line in r.text.strip().split("\n"):
                m = re.search(r'v_(\w+)="(.+)"', line)
    ...
    _today_str_main = datetime.now().strftime("%Y-%m-%d")
    for c in all_candidates:
        klines = kline_map_all.get(c["code"], [])
        if klines and len(klines) >= 2:
            try:
                # 判断最新K线是否为今天
                _latest = str(klines[-1].get("day", ""))[:10]
                _y_idx = -1 if _latest != _today_str_main else -2
                if len(klines) >= abs(_y_idx):
                    yesterday_vol = int(float(klines[_y_idx].get("volume", 0)))
                    if yesterday_vol > 0:
    ...
    for c in filtered:
        ...
        # 用K线数据修正昨收价（日期感知：非交易日最新K线=昨日）
        if klines and len(klines) >= 2:
            try:
                _latest = str(klines[-1].get("day", ""))[:10]
                _y_idx = -1 if _latest != _today_str_main else -2
                if len(klines) >= abs(_y_idx):
                    real_prev_close = float(klines[_y_idx].get("close", 0))
    ...
    # ========== 最终过滤：保留OR逻辑，任一策略池通过即可入选 ==========
    before_count = len(final)
    final = [c for c in final if c.get("passed_pools") and len(c["passed_pools"]) > 0]
    removed = before_count - len(final)
    if removed > 0:
        print(f"\n  🗑  最终过滤: 去除 {removed} 只无策略池命中的股票，保留 {len(final)} 只")
...
def _screen_fallback_all_market() -> list[dict]:
    ...
    for c in filtered:
        ...
        if klines and len(klines) >= 2:
            try:
                _latest = str(klines[-1].get("day", ""))[:10]
                _y_idx = -1 if _latest != _today_str_fb else -2
                if len(klines) >= abs(_y_idx):
                    real_prev_close = float(klines[_y_idx].get("close", 0))
                    if real_prev_close > 0:
                        c["prev_close"] = real_prev_close
