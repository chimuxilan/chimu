    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if attempt < max_retries - 1:
                _time.sleep((2 ** attempt) + random.uniform(0.3, 0.8))
        except requests.exceptions.ConnectionError:
            last_err = "connection error"
            if attempt < max_retries - 1:
                _time.sleep((2 ** attempt) + random.uniform(0.5, 1.5))
...
def fetch_hist(code: str, days: int = 10) -> list[dict]:
    ...
    for attempt in range(3):
        try:
            ...
            pass
        if attempt < 2:
            _time.sleep((2 ** attempt) + random.uniform(0.2, 0.6))
    ...
    for attempt in range(2):
        try:
            ...
            pass
        if attempt < 1:
            _time.sleep(1.0 + random.uniform(0.2, 0.5))
...
def _fetch_sector_kline(sector_code: str, days: int = 10) -> list[dict]:
    """获取板块日K线数据（带缓存）"""
    if sector_code in _sector_kline_cache:
        return _sector_kline_cache[sector_code]
    EM_HEADERS = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
        "Referer": "https://quote.eastmoney.com/center/boardlist.html",
    }
    ...
    try:
        ...
        pass
    _sector_kline_cache[sector_code] = klines
    return klines


def _check_sector_5day_rise(sector_code: str, threshold: float = 5.0) -> bool:
    """检查板块近5日涨幅是否超过阈值"""
    klines = _fetch_sector_kline(sector_code, days=10)
    if len(klines) < 6:
        return False
...
def pool_technical(code: str, klines: list[dict], name: str = "",
                   ...
                   sector: str = "", sector_code: str = "") -> tuple[bool, str]:
    """
    策略池4 · 技术池（多周期MACD共振）
    ─────────────────────────────────
    条件:
...
def _fetch_sectors_with_stocks() -> dict:
    """
    通过东方财富 push2test 接口获取行业板块列表及各板块下的股票（带限速）
    ...
    for sname, limit_cnt in sorted_sectors:
        ...
        for item in stocks:
            ...
            cand = {
                ...
                "turnover_sina": 0,
                "sector": sname,
                "sector_code": sectors.get(sname, {}).get("code", ""),
